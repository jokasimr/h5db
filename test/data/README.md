...
These files are for benchmarking and are **not** generated by the test data script:

- `create_benchmark_data.py` → `benchmark.h5` (27 MB) - Used in root-level benchmark scripts
- `create_large_benchmark.py` → `large_benchmark.h5` - Large benchmark (optional)
- `create_huge_benchmark.py` → `huge_benchmark.h5` - Huge benchmark (optional)
- `create_100m_benchmark.py` → `benchmark_100m.h5` - 100M row benchmark (optional)
- `create_large_test.py` → `large_test.h5` - Unused test file
